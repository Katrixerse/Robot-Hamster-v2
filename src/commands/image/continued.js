...
module.exports = {
    ...
    run: async (bot, prefix, message, args, con) => {
        ...
        try {
            ...
            const data = await loadImage('./src/assets/images/to-be-continued.png');
            const canvas = createCanvas(634, 675);
            const ctx = canvas.getContext('2d');
            ctx.fillRect(0, 0, 634, 675);
            ctx.drawImage(convertImage, 0, 0, 634, 675);
            ctx.drawImage(data, 233, 545, 384, 165);
            const attachment = canvas.toBuffer();
            return message.channel.send({
                files: [{
                    attachment,
                    name: 'to-be-continued.png'
                }]
            });
        } catch (err) {
