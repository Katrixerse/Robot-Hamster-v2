...
module.exports = {
        name: '75',
        aliases: ["75"],
        description: 'Roll, if the number is above 75 you win x1.75',
        usage: '75 <number>',
        cooldownTime: '1',
        group: 'economy',
        botPermissions: ['none'],
        run: async (bot, prefix, message, args, con) => {
            try {
                con.query(`SELECT ServerCash FROM serverSettings WHERE guildId ="${message.guild.id}"`, (e, settings) => {
                  con.query(`SELECT * FROM serverCash WHERE guildId = "${message.guild.id}" AND userId = "${message.author.id}"`, (e, row2) => {
                    ...
                    const won = Math.round(num * 1.75);
                    if (!num) return mainFuncs.sendUsage(message, prefix, `75 <number>`, `number`);
