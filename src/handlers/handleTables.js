...
const createTables = () => {
    ...
    con.query('CREATE TABLE IF NOT EXISTS blacklist (user TEXT(30), reason TEXT(255))');

    // Moderation/config tables
    con.query('CREATE TABLE IF NOT EXISTS serverPrefix (guildId TEXT(30), prefix TEXT(30))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS casenumber (guildId TEXT(30), cn INTEGER(30), m_id TEXT(30))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS serverAutoroles (guildId TEXT(30), enabled TEXT(20), roles TEXT(255))');
    ...
    con.query('CREATE TABLE IF NOT EXISTS serverRep (guildId TEXT(30), userId TEXT(30), reason TEXT(50), reppedBy TEXT(50))');
    con.query('CREATE TABLE IF NOT EXISTS serverTotalRep (guildId TEXT(30), userId TEXT(30), rep INTEGER(30))');
    con.query('CREATE TABLE IF NOT EXISTS serverRolepersist (guildId TEXT(30), userId TEXT(30), role TEXT(255))');
    con.query('CREATE TABLE IF NOT EXISTS serverTicketSettings (guildId TEXT(30), ticketsEnabled TEXT(30), messageId TEXT(30))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS serverTfa (guildId TEXT(30), enabled TEXT(30), code TEXT(50))');
    con.query('CREATE TABLE IF NOT EXISTS serverCustomCommands (guildId TEXT(30), name TEXT(30), output TEXT(255))');
    con.query('CREATE TABLE IF NOT EXISTS serverStaff (guildId TEXT(30), userId TEXT(30), userRank TEXT(30))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS serverTickets (guildId TEXT(30), roles TEXT(255), ticketNumber INTEGER(30))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS userGuilds (userId TEXT(30), guilds TEXT(255))');
    con.query('CREATE TABLE IF NOT EXISTS userPrefGuilds (userId TEXT(30), guild TEXT(255))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS serverTimedRoles (guildId TEXT(30), userId TEXT(30), role TEXT(30), timeLeft INTEGER(30))');

    // Twitch/Reddit
    con.query('CREATE TABLE IF NOT EXISTS streams (userId TEXT(255))');
    con.query(
    ...
    con.query('CREATE TABLE IF NOT EXISTS serverLevelRewards (guildId TEXT(30), levelRequired INTEGER(50), role TEXT(255))');
    con.query(
...
const addGuild = (gname, gid) => {
    con.query('SELECT * FROM serverPrefix WHERE guildId = ?', [gid], (e, rows) => {
        if (e) return console.log(e.message);
        if (!rows || rows.length === 0) {
            con.query('INSERT INTO serverPrefix (guildId, prefix) VALUES (?, ?)', [gid, 'r!']);
            con.query(
            ...
            con.query('INSERT INTO casenumber (guildId, cn) VALUES (?, ?)', [gid, 0]);
            con.query(
            ...
            con.query('INSERT INTO serverAutoroles (guildId, enabled, roles) VALUES (?, ?, ?)', [gid, 'no', '']);
            // Include code value to satisfy serverTfa schema
            con.query('INSERT INTO serverTfa (guildId, enabled, code) VALUES (?, ?, ?)', [gid, 'false', 'none']);
            con.query(
...
const removeGuild = (gname, gid) => {
    con.query('SELECT 1 FROM serverSettings WHERE guildId = ? LIMIT 1', [gid], (e, rows) => {
        if (e) return console.log(e.message);
        if (rows && rows.length >= 1) {
            ...
            tables.forEach((t) => {
                con.query(`DELETE FROM ${t} WHERE guildId = ?`, [gid]);
            });
